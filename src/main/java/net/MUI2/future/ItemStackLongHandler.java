...
import net.MUI2.api.IItemStackLong;
import net.MUI2.future.INBTSerializable;
import net.MUI2.future.ItemHandlerHelper;
import net.MUI2.utils.item.ItemStackLong;
...
public class ItemStackLongHandler implements IItemHandlerLong, INBTSerializable<NBTTagCompound> {
    ...
    public IItemStackLong extractItemLong(int slot, long amount, boolean simulate) {
        ...
        if (existing.getStackSize() <= toExtract) {
            ...
            if (!simulate) {
                this.items.set(
                    slot,
                    net.MUI2.future.ItemHandlerHelper.copyStackWithSize(existing, existing.getStackSize() - toExtract));
                this.onContentsChanged(slot);
            }

            return net.MUI2.future.ItemHandlerHelper.copyStackWithSize(existing, toExtract);
        }
    ...
    public IItemStackLong insertItemLong(int slot, IItemStackLong stack, boolean simulate) {
        if (stack == null) {
            ...
            if (existing != null) {
                if (!net.MUI2.future.ItemHandlerHelper.canItemStacksStack(stack, existing)) {
                    return stack;
                }
            ...
            if (limit <= 0) {
                ...
                if (!simulate) {
                    if (existing == null) {
                        this.items.set(slot, reachedLimit ? net.MUI2.future.ItemHandlerHelper.copyStackWithSize(stack, limit) : stack);
                    } else {
